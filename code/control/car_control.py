...
ABS_MAX_SPEED  = 2.20        # m/s  绝对天花板 = FR-07Pro 硬件规格 8km/h(厂家手册p66的1.5是其Apollo驱动
                             #      软件钳位, 遥控实测2.32可超; 指令勿超硬件顶, 超了只饱和windup拖长刹停)
MAX_STEER      = 25.0        # deg  固件范围 ±25
CMD_TIMEOUT    = 0.30        # s    超过这么久没有新指令就当死人开关触发
BRAKE_ON_STOP  = 25          # 停车/死人开关时给的刹车量 (0~127)
SPEED_SLEW     = 2.6         # m/s/s 速度变化兜底限幅(防猛冲), 0=不限。加速实际由控制器 ACCEL_UP(0.9)管;
                             # 须≥控制器 ACCEL_DOWN(2.5)否则把刹车卡慢——停车要快, 安全优先
...
class CarController(object):
    ...
    def __init__(self, channel="can0", bustype="socketcan",
        ...
        self._target_gear = GEAR_DRIVE
        self._cur_gear = GEAR_DRIVE
    ...
    # ---- 对外指令接口 (跟随循环调用) ----
    def set_cmd(self, speed_mps, steer_deg, gear=GEAR_DRIVE):
        """更新期望(速度, 转向, 档位)。线程安全。steer_deg 为 fr 原生符号(负=右)。
        档位切换有安全互锁: 只在车速≈0 时真正换挡(见 _loop), 换挡前速度自动压 0。"""
        with self._lock:
            self._target_speed = float(speed_mps)
            self._target_steer = float(steer_deg)
            self._target_gear = int(gear)
            self._last_cmd_t = time.monotonic()

    def set_cmd_from_follow(self, speed_mps, steer_ctrl_deg, gear=GEAR_DRIVE):
        """跟随控制器专用: steer_ctrl_deg 为控制器符号(正=右), 内部取负。"""
        self.set_cmd(speed_mps, STEER_SIGN * steer_ctrl_deg, gear=gear)
    ...
    def _loop(self):
        ...
        while self._running:
            ...
            # 换挡互锁: 目标档位变了 → 先把速度压 0, 车停稳(<0.05)才真正切档
            if tgt_gear != self._cur_gear:
                if self._cur_speed < 0.05:
                    self._cur_gear = tgt_gear
                else:
                    tgt_speed = 0.0        # 减速中, 本周期不换挡
            ...
            data = encode_ctrl_cmd(self._cur_speed, tgt_steer, gear=self._cur_gear,
