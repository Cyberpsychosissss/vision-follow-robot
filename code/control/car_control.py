...
安全设计 (本脚本的硬约束):
  ...
  - **硬限速**: MAX_SPEED 默认 0.40 m/s; 绝对天花板 ABS_MAX_SPEED=0.60 m/s, CLI 也越不过 (远低于固件 1.5 上限)。
...
# ---------------- 安全包络 (硬限) ----------------
MAX_SPEED      = 0.40        # m/s  默认硬限速 (跟随用, 室内慢速)
ABS_MAX_SPEED  = 0.60        # m/s  绝对天花板, 任何入参/CLI 都不得超过
MAX_STEER      = 25.0        # deg  固件范围 ±25
CMD_TIMEOUT    = 0.30        # s    超过这么久没有新指令就当死人开关触发
BRAKE_ON_STOP  = 25          # 停车/死人开关时给的刹车量 (0~127)
SPEED_SLEW     = 0.6         # m/s per s  速度变化限速(防猛冲), 0=不限
