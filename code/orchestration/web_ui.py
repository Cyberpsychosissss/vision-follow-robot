...
GRAB_CMD = ("docker exec -d %s bash -c 'cd /apollo/follow_data/zkhy_grab && "
            ...
            "./zkhy_grabber --out-dir /apollo/follow_data/runtime/grab --duration 0 --write-fps 5 "
...
YOLO_CMD = ("docker exec -d %s bash -c 'cd /apollo/follow_data/trtx/build && "
            ...
            "--runtime /apollo/follow_data/runtime --hz 10 > /tmp/yolo_follow.log 2>&1'") % CONTAINER
...
def _recorder_loop(host_run, stop_ev):
    ...
    period = 1.0 / REC_FPS
    last_img_m = last_dsp_m = last_obs_m = None
