...
规格：前后探头 90°水平/45°垂直/4 m，侧边探头 50°/50°/4 m，8 度支架。

**两件事没定，必须在空地做**（当前停车位置有静态近距回波干扰）：
1. **单位**：观测最大值 124。若是 cm 则最远只能表示 2.54 m < 标称 4 m → **大概率 2cm/LSB**（254×2=508cm ⊇ 4m）。需站在量好的距离上标定。
...
触发条件（**全部满足**）：
- 面板「允许召唤」开关 ON（**默认关**）
- `gesture.json` 新鲜且 wave 有效
- **挥手者 ReID 匹配 gallery**（cos ≥ `REACQ`）—— 路人招手不响应
- 连续 1.5 s 确认

与 FOLLOW 的差异：
| | 值 |
|---|---|
| 限速 | `HAIL_SPEED = 0.6 m/s`（低于 max_speed） |
| 远距限制 | `FAR_LIMIT` 放宽到 15 m（召唤本来就远），但必须 ReID 确认 |
| 到达判据 | `DESIRED_MIN + 0.3` → 自动转 FOLLOW 并锁定该人 |
