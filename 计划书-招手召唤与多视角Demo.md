...
### 2026-07-20 晚 — P1 传感器接入层完成（P1.1/P1.2/P1.4）

- ✅ **P1.1 C16 格式摸清**：垂直角表取自车上 `lslidar_decoder.h::SCAN_ALTITUDE`（权威，没自己猜），换算是经典 VLP-16 交错排布 `-15,+1,-13,+3,…,-1,+15`。抓 95 个真实包验证：1140 个 block 头全部 `FF EE`，方位角单调、增量中位 0.360°，厂商码 `37 10`。
- ✅ **P1.2 `code/perception/lslidar_c16_udp.py`**：宿主机直收 UDP 解点云。
  - **几何验证（最强证据）**：用真实数据渲俯视图，点云画出一个约 55×35 m 的**矩形室内空间，四面墙笔直、墙角成直角、天花板呈同心弧**。垂直角表/方位角插值/距离标度/坐标变换任一处出错都会让墙变弯。
  - 车上实时冒烟：稳定 **83~84 包/圈、约 3 万点/圈**，圈间隔 0.106/0.097 s = **10 Hz**，与 `rpm=600` 吻合。
  - **副产物：搞清了车停在哪** —— 大型室内空间。这解释了最近点 5.55 m、以及**完全没有地面回波**（室内平滑地面在掠射角下镜面反射）。
- ✅ **P1.4 `code/perception/ars408_can.py`**：毫米波目标列表解码，零驱动安装。
  - 位域验证：车停室内静止场景，解出**所有目标 Vlong=+0.00 / Vlat≈0 / 动态属性=静止** —— 两个独立字段同时符合预期。车上实时：一帧 12 目标，最近 3.4 m/+0.0 m。
- ✅ 传输工具化：`scratchpad/push_file.sh`（base64 走 `ssh 'bash -s'`，带 md5+py_compile 门），以后传文件直接用它。

**🔴 P1.6 外参标定需要你配合**：试图用地面回波自标定**失败**（无近场回波），且近场 15 m 内**车身自己也无回波**（雷达装的位置看不到自己的车）→ 没有任何可用参考特征。必须做实物实验：**车头正前方量好距离放个纸箱/纸板，我抓一圈点云看它落在哪**。2 分钟，一次搞定安装高度与朝向。

**下一步**：P1.3（高度过滤 + 占据栅格 + CAN 里程滚动）→ P1.5（mini-map 绘制）→ P1.6（需你配合）。
